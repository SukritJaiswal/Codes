...
if (prod[i] = 1.62)
{
	double dp_surf1[200]
	double dp_surf2
	for (omega = 0.01,omega < 2;omega = omega + 0.01)
	{
		dp_surf2 = omega2*pow(rr,2.)*ela*v/e2phi+dphi*w/dr;
		dp_surf1[omega] = dp_surf2
		print(omega,dp_surf1[omega])
	}
}
...
double dr=1.e-3;
double dm,dp,dphi,dphidr,dedp,xmr,ela,e2phi;
